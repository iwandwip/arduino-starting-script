...
void SensorModule::debug(int __index = -1) {
        if (base == nullptr) return;
...
void SensorDebug::addLabel(String _label, bool with_separator = true) {
...
void SensorDebug::setSeparator(String _sep) {
        separator = _sep;
}
...
void SensorDebug::addFloatData(String varName, float value, bool with_separator = true, int precision = 2) {
        if (with_separator && data != "") data += ",";
        data += varName + ":" + String(value, precision);
}

void SensorDebug::addIntData(String varName, int value, bool with_separator = true) {
        if (with_separator && data != "") data += ",";
        data += varName + ":" + String(value);
}

void SensorDebug::addBoolData(String varName, bool value, bool with_separator = true) {
        if (with_separator && data != "") data += ",";
        data += varName + ":" + String(value ? "true" : "false");
}

void SensorDebug::addStringData(String varName, String value, bool with_separator = true) {
        if (with_separator && data != "") data += ",";
        data += varName + ":" + value;
}